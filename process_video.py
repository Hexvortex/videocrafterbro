...
from elevenlabs import Voice, VoiceSettings, save as save_11
...
def generate_srt_file(audio_file_path, text_file_path, output_srt_file_path):
    ...
    command = (
        f'python3.10 -m aeneas.tools.execute_task "{audio_file_path}" "{text_file_path}" '
...
def convert_text_to_speech(
    text_file_path, voice_id, api_key, output_audio_file="audio_files/output.mp3"
):
    """
...
def speed_up_video_with_audio(
    input_video, output_video_path, speed_factor, textfile_id
):
...
def add_subtitles_to_clip(
    ...
    font_path = os.path.join(
        os.getcwd(), "fonts", "31692f02-5637-4cd2-b973-99a09e542b83.ttf"
    )
    if margin is None:
...
def convert_to_webm(input_video_path, output_webm_path):
    ...
    video_clip.write_videofile(
        output_webm_path, codec="libvpx", audio_codec="libvorbis"
    )
...
def main():
    ...
    output_audio_file = os.path.join(
        base_path, "text_audio", f"{textfile_id}_converted_audio.mp3"
    )
    ...
    output_srt_file_path = os.path.join(
        base_path, "srt_files", f"{textfile_id}_generated_srt_output.json"
    )
    ...
    blank_vide = generate_blank_video_with_audio(
        audio_file, srt_file_path, blank_vide_path, resolution
    )
    ...
    print(subtitles)
    blank_video_segments, subtitle_segments = get_segments_using_srt(
    ...
    for replacement_video_file in replacement_video_files:
        ...
        cropped_replacement_video = crop_to_aspect_ratio_(
    ...
    final_blank_video = concatenate_clips(
    ...
    final_video_speeded_up = os.path.join(
        base_path, "tmp", f"output_variation{textfile_id}_speed-up.mp4"
    )
    final_video_speeded_up_clip = speed_up_video_with_audio(
        final_video, final_video_speeded_up, 1, textfile_id
    )
    update_progress(70, dir_s)

    # Output file
    watermarked_path = os.path.join(
        base_path, "final", "w", f"final_output_{textfile_id}.mp4"
    )
