...
openai.api_key = (
    "sk-proj-mo9iZjhl3DNjXlxMcx1FT3BlbkFJz5UCGoPBLnSQhh2b2stB"  # write your openai api
)
PEXELS_API_KEY = (
    "ljSCcK6YYuU0kNyMTADStB8kSOWdkzHCZnPXc26QEHhaHYqeXusdnzaA"  # write your pexels
)
...
print(
    local_video_paths
)  # This will print the dictionary of folder names and their paths
...
def get_best_hd_file(videos):
    ...
    for video in videos:
        for file in video["video_files"]:
            ...
            print(
                f"Evaluating file: Width: {width}, Height: {height}, Aspect Ratio: {aspect_ratio}, Likes: {likes}"
            )
            ...
            if (
                width >= 1280
                and height >= 720
                and abs(aspect_ratio - desired_aspect_ratio) <= 0.2
            ):
            ...
            if not best_file:
                ...
                if (width >= 1024 and height >= 576) or (
                    aspect_ratio >= 1.5 and width >= 1024
                ):
...
def generate_blank_video_with_audio(
    audio_file, srt_file, output_file, resolution="16:9"
):
...
def embed_in_background(
    video: VideoFileClip, target_resolution: tuple
) -> VideoFileClip:
    """
    ...
    final_clip = CompositeVideoClip(
        [background, resized_video.set_position((x_center, y_center))]
    )
    final_clip.write_videofile(
        os.path.join(os.getcwd(), "cropped", "video.mp4"),
        codec="libx264",
        audio_codec="aac",
    )
...
def add_subtitles_to_clip(
    ...
    color: str = "white",
    margin: int = 30,
    font_path: str = os.path.join(os.getcwd(), "data", "Montserrat-SemiBold.ttf"),
) -> VideoFileClip:
    ...
    for replace_index, replacement_video in replacement_videos.items():
        if 0 <= replace_index < len(combined_segments):
            ...
            adjusted_segment_with_subtitles = add_subtitles_to_clip(
                ...
                font_customization[1],
                int(font_customization[4]),
                font_customization[0],
            )
...
def add_background_music(
    final_video: VideoFileClip, background_music_file: str
) -> VideoFileClip:
    try:
        ...
        final_audio = CompositeAudioClip(
            [final_video.audio, background_music.volumex(0.1)]
        )
...
def main():
    ...
    try:
        ...
        with open(script_file, "r") as text_file:
            ...
            )  # Read and remove leading/trailing whitespace
            print(text)
    ...
    audio_file = os.path.join(
        os.getcwd(), "tmp", "output_of_lead_kyrona.mp3"
    )  # path for mp3 file
    output_video_file = os.path.join(
        os.getcwd(), "tmp", "output_video.mp4"
    )  # path for the output video
    replacement_base_folder = os.path.join(
        os.getcwd(), "tmp", "downloads"
    )  # path of a base directory where videos will be downloaded
    # if music_path_:
    ...
    try:
        ...
        if os.path.exists(replacement_base_folder) == True and os.path.isdir(
            replacement_base_folder
        ):
    ...
    for line in category_lines:
        ...
        local_video_paths[topic_folder].append(
            os.path.join(topic_folder, selected_video)
        )
    ...
    for index in range(num_segments):
        if index < len(category_lines):
            ...
            logging.debug(
                f"Processing segment {index + 1}: Video: {video}, Category: {category}"
            )
            ...
            if line and category != "No Category":
                ...
                if category_path:
                    ...
                    logging.debug(
                        f"Found {len(video_files)} unprocessed local video(s) for category '{category}'"
                    )
                else:
                    video_files = []
                    logging.warning(
                        f"No local video files found for category '{category}' or directory does not exist"
                    )

                if video_files:
                    video_file = os.path.join(
                        video_folder, video_files[0]
                    )  # Use the first unprocessed file
                    segment_folder = os.path.join(output_base_folder, str(index + 1))
                    os.makedirs(segment_folder, exist_ok=True)
                    output_file = os.path.join(
                        segment_folder, os.path.basename(video_files[0].split("/")[1])
                    )
                    try:
                        shutil.copy(
                            video_file, output_file
                        )  # Copy the local video to the segment folder
                        logging.info(
                            f"Using local video: {video_file} for segment {index + 1}"
                        )
                        ...
                        logging.error(
                            f"File not found: {video_file}. Skipping to next file."
                        )
                else:
                    logging.warning(
                        f"No local video files found for category '{category}' or directory does not exist"
    ...
    for index, line in unclassified_lines:
        ...
        if result:
            ...
            if best_video and best_file:
                ...
                if (
                    output_file not in processed_videos
                ):  # Ensure not to download same video multiple times
    ...
    try:
        for missing_index in missing_segments:
            if missing_index <= len(lines_with_categories):
                ...
                logging.warning(
                    f"Missing video for segment {missing_index}, retrying..."
                )
                ...
                if result:
                    ...
                    if best_video and best_file:
                        ...
                        segment_folder = os.path.join(
                            output_base_folder, str(missing_index)
                        )
                        os.makedirs(segment_folder, exist_ok=True)
                        safe_filename = (
                            f'{line[:10].replace(" ", "_").replace("/", "_")}.mp4'
                        )
                        output_file = os.path.join(segment_folder, safe_filename)
                        if (
                            output_file not in processed_videos
                            ...
                            processed_videos.add(
                                output_file
                            )  # Track the processed video
                            downloaded_segments.add(missing_index)
                    else:
                        logging.warning(
                            f"No suitable HD video found for keywords: {keywords}"
                        )
                else:
                    logging.warning(f"No results found for keywords: {keywords}")
    except Exception as e:
    ...
    for folder in os.listdir(replacement_base_folder):
        ...
        for replacement_video_file in replacement_video_files:
            ...
            cropped_replacement_video = resize_to_aspect_ratio(
                replacement_video, MAINRESOLUTIONS[resolution]
    ...
    print(
        "the replacement_videos_per_combination: ", replacement_videos_per_combination
    )
    update_progress(65)
    ...
    final_video = concatenate_clips(
        video_segments, target_resolution=RESOLUTIONS[resolution], target_fps=30
    )
    ...
    for i, replacement_videos in enumerate(replacement_videos_per_combination):
        ...
        concatenated_video = concatenate_clips(
            final_video_segments,
            target_resolution=RESOLUTIONS[resolution],
            target_fps=30,
        )
        original_audio = video.audio.subclip(
            0, min(concatenated_video.duration, video.audio.duration)
        )
        ...
        final_video_speeded_up = os.path.join(
            os.getcwd(), "tmp", f"output_variation{i + 1}_speed-up.mp4"
        )
        final_video_speeded_up = speed_up_video_with_audio(
            final_video, final_video_speeded_up, speed_factor=1
        )
        output_file = os.path.join(
            os.getcwd(), "static", "final", f"output_variation{i + 1}.mp4"
        )
        final_video_speeded_up.write_videofile(
            os.path.normpath(output_file), codec="libx264", audio_codec="aac"
        )
